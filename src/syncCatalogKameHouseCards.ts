import { crawlKameHouseCardsListing } from "./helpers/kameHouseCardsCategoryCrawler.js";
import { getAllTargetsForSite, insertTargets } from "./helpers/db.js";
import { notifyTelegram } from "./helpers/notify.js";

// Scoped to in-stock Pokémon products only (per how this URL was chosen).
const LISTING_URL = "https://kamehousecards.com/collections/pokemon?filter.v.availability=1&sort_by=manual&grid=default";
...
async function notifyNewProducts(products: { url: string; name: string }[]): Promise<void> {
  ...
  const lines = [
    `🆕 <b>[KameHouseCards] ${products.length} new product${products.length > 1 ? "s" : ""} found</b>`,
    ...shown.map((p) => `• ${p.name}\n${p.url}`),
  ];
...
export async function runSyncCatalogKameHouseCards(): Promise<void> {
  const listed = await crawlKameHouseCardsListing(LISTING_URL);
  const listingByUrl = new Map(listed.map((p) => [p.url, p.name]));

  const existing = await getAllTargetsForSite("kamehousecards");
  ...
  await insertTargets(newProducts.map((p) => ({ site: "kamehousecards" as const, url: p.url, name: p.name })));

  console.log(`Catalog sync (KameHouseCards): ${listingByUrl.size} listed (available now), ${newProducts.length} new`);
